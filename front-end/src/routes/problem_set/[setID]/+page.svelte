...
<div class="flex flex-col min-h-screen items-center">
    <center class="m-2" style="font-size:30px;font-weight:bold;">{title}</center>
    <div class="overflow-x-auto border-2 rounded-xl m-5">
        ...
        <tbody>
          {#each problems as item (item.id)}
            <tr>
              ...
              <td>{item.id}</td>
              <!-- <td><a target="_blank" href = {item.source=="Codeblaze"?"/problem/"+item.pID:item.link}>{item.title}</a></td> -->
              <td id="prob_button" class="button" on:click={window.open(item.source=="Codeblaze"?"/problem/"+item.pID:item.link,"_blank")}>{item.title}</td>
              <td>
                {#if item.type == "description"}
                    <img style = "width:30px;height:30px" src="https://img.icons8.com/external-inipagistudio-mixed-inipagistudio/64/external-documentation-business-process-inipagistudio-mixed-inipagistudio.png"/>
                {:else if item.source == "Codeblaze"}
                    <img style = "width:30px;height:30px" src = "/favicon-32x32.png"/>
                {:else if item.source == "Leetcode"}
                    <img style = "width:30px;height:30px" src = "https://assets.leetcode.com/static_assets/public/icons/favicon-16x16.png"/>
                {:else if item.source == "Codeforces"}
                    <img style = "width:30px;height:30px" src = "https://codeforces.org/s/67471/favicon-16x16.png"/>
                {:else if item.source == "Codechef"}
                    <img style = "width:30px;height:30px" src = "https://s3.amazonaws.com/codechef_shared/favicon.ico"/>
                {:else}
              ...
              {#if item.difficulty == "Easy"}
                <td style="color:lime">{item.difficulty}</td>
              {:else if item.difficulty == "Medium"}
                <td style="color:yellow">{item.difficulty}</td>
              {:else if item.difficulty == "Hard"}
                <td style="color:red">{item.difficulty}</td>   
              {/if}
