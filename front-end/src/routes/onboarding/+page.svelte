<script>
    ...
    let user_name,country,SESSION_USER;
    const unsubscribe = session_user.subscribe((value) => {
        SESSION_USER = value;
    });
    let onBoard = async function(){
            const { error } = await supabase
  .from('users')
  .update({ name: user_name,country:country,onboarding:true})
    ...
    <div class="card shrink-0 w-full max-w-sm shadow-2xl bg-base-100">
        <form class="card-body">
            ...
            <div class="form-control mt-6">
                <button class="btn btn-primary" on:click={onBoard}>Join</button
                >
            </div>
        </form>
    </div>
